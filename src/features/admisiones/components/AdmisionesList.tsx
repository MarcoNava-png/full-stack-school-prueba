...
/** NUEVO: prop opcional para doble click en fila */
export type AdmissionsListProps = {
  onRowDoubleClick?: (a: AdmissionsItem) => void;
};
...
export function AdmissionsList({ onRowDoubleClick }: AdmissionsListProps) {
  ...
  const filteredAdmissions = useMemo<AdmissionsItem[]>(() => {
    ...
    return list.filter(a =>
      ...
      a.status?.toLowerCase().includes(term) ||
      a.email?.toLowerCase().includes(term) ||
      a.programs?.some(p => p.name?.toLowerCase().includes(term)) ||
      a.plans?.some(pl => pl.name?.toLowerCase().includes(term))
    );
  }, [admissions, searchTerm]);
  ...
  // Prellenado básico para el modal al editar (si procede)
  const splitFullName = (full: string) => {
    const parts = (full || '').trim().split(/\s+/);
    if (parts.length === 0) return { nombre: '', apellidoPaterno: '', apellidoMaterno: '' };
    if (parts.length === 1) return { nombre: parts[0], apellidoPaterno: '', apellidoMaterno: '' };
  ...
  const mapToAdmissionFormData = (a: AdmissionsItem): AdmissionPayload => {
    ...
    return {
      ...
      apellidoPaterno,
      apellidoMaterno,
      fechaNacimiento: new Date().toISOString().split('T')[0], // default
      calle: '',
      numero: '',
      codigoPostalId: 0,
      personaGeneroId: 1,
      // planEstudiosId: a.plans?.[0]?.id, // si quieres precargar cuando exista
    };
  };
  ...
  // Formatea fecha sin locales (evita hydration mismatch)
          ...
          <TableBody>
            {(filteredAdmissions ?? []).map((a) => (
              <TableRow
                ...
                /** NUEVO: doble click -> avisa al padre */
              ...
              >
                ...
                <TableCell className="text-sm text-gray-700">
                  {a.programs?.length ? a.programs.map(p => p.name).join(', ') : '—'}
                </TableCell>

                <TableCell className="text-sm text-gray-700">
                  {a.plans?.length ? a.plans.map(p => p.name).join(', ') : '—'}
                </TableCell>
